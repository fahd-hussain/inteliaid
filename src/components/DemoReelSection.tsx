import { useRef, useState, forwardRef, HTMLAttributes } from 'react';
...
const DemoReelSection = forwardRef<HTMLDivElement, HTMLAttributes<HTMLDivElement>>(
  ({ className = '', ...rest }, ref) => {
    ...
    const handlePlay = (target: 'main' | number) => {
      ...
      else cardRefs.current.forEach((el) => el?.pause());

      if (typeof target === 'number') {
        cardRefs.current.forEach((el, i) => {
          if (el && i !== target) el.pause();
        });
      }

      setActiveVideo(target);

      // Wait for state update before playing selected video
      setTimeout(() => {
        if (target === 'main') {
    ...
    return (
      <section ref={ref} className={`py-20 px-4 bg-dark-bg ${className}`} {...rest}>
        <div className="max-w-6xl mx-auto">
          ...
          {/* Main Demo Reel Video with Preview */}
...
DemoReelSection.displayName = 'DemoReelSection';
